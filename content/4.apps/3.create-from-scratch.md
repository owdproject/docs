...
> [!TIP]
> **Scaffold Automatically with OWD CLI**
> If you are working inside an OWD workspace, you can automatically scaffold a new application using the OWD CLI command:
> ```bash
> pnpm desktop create app my-app
> ```
> This command will copy the `app-template` blueprint, rename all files/directories, replace placeholders, link the package in `desktop/package.json`, register it in `desktop/desktop.config.ts`, and run `dev:prepare`.
>
> You can also do this directly from the interactive Control Panel (`pnpm desktop`) by pressing **`n`** (or choosing **Create new app or module** from the menu) and following the interactive prompts.
