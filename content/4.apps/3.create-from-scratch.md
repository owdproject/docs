...
flowchart TD
  ...
  appConfig[runtime/app.config.ts]
  registerPlugin[runtime/plugin.ts]
  windowComp[Window component]
  playground[playground + desktop.config.ts]
  launchPlugin[optional launch-*.client.ts]
  validate[desktop validate + pnpm dev]
  desktopWire[wire into client/desktop]

  scaffold --> moduleTs --> appConfig --> registerPlugin --> windowComp
  windowComp --> playground --> launchPlugin --> validate --> desktopWire
```
