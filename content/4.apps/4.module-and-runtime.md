...
- **`meta.name`**: stable module id (e.g. `owd-app-about`).
...
export default defineDesktopModule({
  meta: { name: 'owd-app-about', configKey: 'about' },
...
- sets **`name: 'owd-<slug>-register'`** (for playground launch `dependsOn`)
...
The `@owdproject/kit-primevue` package aggregates these paths and merges them during Nuxt boot into the Tailwind CSS configuration.
