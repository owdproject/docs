...
| Term | Meaning |
|------|---------|
| **Core / kernel** | **`@owdproject/core`** — engine Nuxt module, window manager, kernel **`Desktop*`** components. |
| **Kit** | Shared package in **`packages/kit-*`** (e.g. **`kit-theme`**, **`kit-fs`**) — optional glue above core. |
...
| **`defineDesktopApp`** | Registers an application with the application manager. |
| **`ApplicationConfig`** | Schema: id, windows, entries, commands. |
| **`DesktopCore`** | Kernel shell wrapper component (formerly **`CoreDesktop`**). |
| **`useExplorerStore`** | Explorer UI state in **`@owdproject/kit-fs`**. |
| **`useDesktopDialogs`** | Dialog provider accessor in **`@owdproject/kit-theme`**. |
