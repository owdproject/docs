...
| **n** | Create new app or module (interactive wizard) |
| **q** / **Esc** | Quit |
...
Scaffold a new app or module inside an existing OWD workspace. It copies the corresponding blueprint template, renames all files/directories, replaces placeholders, links the package in `desktop/package.json`, registers it in `desktop/desktop.config.ts`, and runs `dev:prepare`.

```bash
desktop create app my-app
desktop create module my-module
```

If you omit the package kind and/or name, the CLI will prompt you interactively:
