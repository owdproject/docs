...
Explorer: **`useExplorerStore`** from **`@owdproject/kit-fs`** (alias **`useDesktopExplorerStore`** deprecated).
...
| Package | Layer |
|---------|--------|
| **`@owdproject/kit-theme`** | Dialogs, workspace edge drop |
| **`@owdproject/kit-fs`** | Explorer UI + store |
| **`@owdproject/module-fs`** | ZenFS VFS |
| **`@owdproject/module-persistence`** | Optional persistence |

## Versioning

Breaking contract changes require semver coordination with themes (separate repos). Current kernel release line: **3.3.x** (`Desktop*` rename, explorer eviction from core, **defu** config merge, config allowlist).
